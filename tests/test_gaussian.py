...
def test_gaussian3d_evalsh(buffer_shape: Tuple[int]):
    ...
    assert np.all(result[:, 1] == pytest.approx(0.0)), "Green channel should be zero"
    assert np.all(result[:, 2] == pytest.approx(0.0)), "Blue channel should be zero"
...
def test_gaussian3d_eval(buffer_shape: Tuple[int]):
    ...
    assert np.all(result[:, 3] == pytest.approx(expected_alpha)), "Alpha should be sigmoid(opacity_logit) * rho"
    assert np.all(result[:, 1] == pytest.approx(expected_base[1])), "Green should match sigmoid(g)"
    assert np.all(result[:, 2] == pytest.approx(expected_base[2])), "Blue should match sigmoid(b)"
    assert np.all(result[:, 0] > expected_base[0]), "Red should include SH contribution"
