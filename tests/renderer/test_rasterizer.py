...
def setup_rasterize_data(benchmark_gaussian_count: int, camera: Camera):
    ...
    cloud = GaussianCloud()
    cloud.randomize(
        benchmark_gaussian_count, position_var=5.0, scale_offst=0.05
    )
    ...
    num_viewing = int(
        inside_offset_cursor[benchmark_gaussian_count - 1].read()
    )  # pyright: ignore

    if num_viewing == 0:
        # Skip test if no gaussians are visible
        pytest.skip(
            "No gaussians are visible after projection, skipping rasterization test."
        )
    ...
    from gslang.radix_sort import numpy_sort
...
def run_rasterize_kernel(data):
    ...
    data["kernel"].dispatch(
        thread_count=[
            data["camera"].sensor_size.x,
            data["camera"].sensor_size.y,
            1,
        ],
        vars={
...
def test_rasterize_benchmark(benchmark: BenchmarkFixture, setup_rasterize_data):
    ...
    assert np.any(
        rendered_image > 0
    ), "Rendered image should contain some pixel data"
...
def test_bwd_rasterize_benchmark(
    benchmark: BenchmarkFixture, setup_bwd_rasterize_data
):
    ...
    grad_buffer_data = (
        setup_bwd_rasterize_data["a_gaussian_2d_sorted_grad_buf"]
        .to_numpy()
        .view(np.float32)
    )
    assert np.any(
